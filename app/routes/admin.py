...
async def get_today_all(employee_id: int, service: AttendanceService = Depends(get_service)):
    records = await service.get_today_records(employee_id)
    if not records:
        raise HTTPException(status_code=404, detail="오늘 출퇴근 기록 없음")
    return records


@router.get("/work-hours", summary="직원의 오늘 근무시간 조회", response_model=WorkHoursResponse)
...
@router.patch("/{employee_id}", summary="출퇴근 기록 수정 (최근 기록)", response_model=AttendanceRecordResponse)
...
@router.delete("/{employee_id}", summary="출퇴근 기록 삭제 (최근 기록)")
